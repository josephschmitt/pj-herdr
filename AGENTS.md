...
`pj-herdr` is a [herdr](https://herdr.dev) plugin that bridges [pj](https://github.com/nickel-org/pj) (project jumper) with herdr workspaces. Pressing `Ctrl-s Shift+P` opens an overlay pane with a fuzzy project picker; selecting a project creates or switches to its herdr workspace.
...
- `[[keys.command]]` — binds `prefix+shift+p` to `pj.open-picker`
...
# Trigger the picker
# Press Ctrl-s Shift+P inside herdr
```
